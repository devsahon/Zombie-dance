...
export function sanitizeModelResponse(rawResponse: string): string {
  ...
  const forbidden = /(alibaba\s*cloud|created\s+by\s+alibaba|developed\s+by\s+alibaba)/i;
  if (!forbidden.test(text)) return text;

  const withoutForbidden = text
    .replace(/alibaba\s*cloud/gi, '')
    .replace(/created\s+by\s+alibaba/gi, '')
    .replace(/developed\s+by\s+alibaba/gi, '');

  if (withoutForbidden.includes(ZOMBIECODER_FIXED_IDENTITY_BN)) {
    return withoutForbidden.replace(/\s{2,}/g, ' ').trim();
  }

  return `${ZOMBIECODER_FIXED_IDENTITY_BN}\n\n${withoutForbidden}`.replace(/\s{2,}/g, ' ').trim();
}
